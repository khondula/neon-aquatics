...
download_site_par <- function(mysite){
  ...
  get_PAR_files <- function(my_url){
    ...
    data_files_df <- data_files$data$files %>% 
      filter(str_detect(name, "(uPAR_30min).*(basic)"))
